...
$params = array_merge( wolmart_wpb_filter_element_params( $params, 'wpb_wolmart_posts_slider' ), wolmart_get_wpb_design_controls(), wolmart_get_wpb_extra_controls() );

vc_map(
	array(
		'name'            => esc_html__( 'Posts Slider', 'wolmart-core' ),
		'base'            => 'wpb_wolmart_posts_slider',
		'icon'            => 'wolmart-icon wolmart-icon-posts-slider',
		'class'           => 'wolmart_posts wolmart_posts_slider',
		'content_element' => true,
		'controls'        => 'full',
		'category'        => esc_html__( 'Wolmart', 'wolmart-core' ),
		'description'     => esc_html__( 'Create wolmart posts with slider layout.', 'wolmart-core' ),
		'params'          => $params,
	)
);
...
add_filter( 'vc_autocomplete_wpb_wolmart_posts_slider_categories_callback', 'wolmart_wpb_shortcode_category_id_callback', 10, 1 );
add_filter( 'vc_autocomplete_wpb_wolmart_posts_slider_categories_render', 'wolmart_wpb_shortcode_category_id_render', 10, 1 );

// Post Ids Autocomplete
add_filter( 'vc_autocomplete_wpb_wolmart_posts_slider_post_ids_callback', 'wolmart_wpb_shortcode_post_id_callback', 10, 1 );
add_filter( 'vc_autocomplete_wpb_wolmart_posts_slider_post_ids_render', 'wolmart_wpb_shortcode_post_id_render', 10, 1 );

if ( class_exists( 'WPBakeryShortCode' ) ) {
	class WPBakeryShortCode_WPB_Wolmart_Posts_Slider extends WPBakeryShortCode {
	}
}
