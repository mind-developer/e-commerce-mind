...
(function ($) {
	...
	Wolmart.initScrollTopButton = function () {
		...
		if (domScrollTop) {
			Wolmart.$body.on('click', '#scroll-top', function (e) {
				Wolmart.scrollTo(0);
				e.preventDefault();
			})
		...
		ProductGallery.prototype.initZoom = function () {
			if (wolmart_vars.single_product.zoom_enabled) {
				...
				if (!this.$wc_gallery.closest('.product-quickview').length && !this.$wc_gallery.closest('.product-widget').length) {
					var buttons = '<button class="product-gallery-btn product-image-full w-icon-zoom"></button>' + (this.$wc_gallery.data('buttons') || '');
	...
	Wolmart.initProductSingle = (function () {
		...
		function initVariableProduct() {
			...
			// Variation
			Wolmart.$body.on('click', '.variations .product-variations button', onClickListVariation)
	...
	Wolmart.initAccount = function () {
		...
		Wolmart.$body
			.on('click', '.header .account > a:not(.logout)', launchPopup)
	...
	Wolmart.initAsync = function () {
		...
		setTimeout(function () { Wolmart.initSlider('.slider-wrapper') });                       // Initialize slider
