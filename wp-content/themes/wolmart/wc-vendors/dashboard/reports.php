...
<h2 class="title section-title"><?php _e( 'Sales Report', 'wc-vendors' ); ?></h2>
...
<table class="table table-condensed table-vendor-sales-report mb-8">
	<thead>
	<tr>
		...
		<?php if ( ! empty( $vendor_summary['products'] ) ) : ?>
			...
			foreach ( $vendor_summary['products'] as $product ) :
				...
				<tr>
					<td class="product">
						<strong>
							<a href="<?php echo esc_url( get_permalink( $_product->get_id() ) ); ?>"><?php echo esc_html( $product['title'] ); ?></a>
						</strong>
						<?php
					...
					<?php if ( $can_view_orders ) : ?>
						<td>
							<a href="<?php echo esc_url( $product['view_orders_url'] ); ?>"><?php _e( 'Show Orders', 'wc-vendors' ); ?></a>
						</td>
					<?php endif; ?>
				</tr>
			<?php endforeach; ?>
