...
<p style="margin-bottom: 30px;"><a href="#" class="button button-large button-light btn-remove-demo-contents"><?php esc_html_e( 'Uninstall Demo', 'wolmart' ); ?><i class="far fa-trash-alt" style="margin-<?php echo is_rtl() ? 'right' : 'left'; ?>: .5rem"></i></a></p>
<div class="wolmart-remove-demo mfp-hide">
	...
	<div class="wolmart-install-section wolmart-wrap" style="margin: 30px 20px; border: none;">
		...
		<div style="flex: 0 0 60%; max-width: 60%;  box-sizing: border-box;">
			...
			<button class="button button-primary button-large" <?php disabled( empty( get_option( 'wolmart_demo_history', array() ) ) ); ?> style="width: 100%"><i class="far fa-trash-alt" style="margin-right: .5rem"></i><?php esc_html_e( 'Uninstall', 'wolmart' ); ?></button>
		</div>
	</div>
</div>
					...
					<?php if ( Wolmart_Admin::get_instance()->is_registered() ) : ?>
						<div class="wolmart-install-editors">
							<label for="wolmart-elementor-demo" class="d-none">
								<input type="radio" id="wolmart-elementor-demo" name="wolmart-import-editor" value="elementor" checked="checked">
							...
							<label for="wolmart-js_composer-demo" class="d-none">
								<input type="radio" id="wolmart-js_composer-demo" name="wolmart-import-editor" value="js_composer">
		...
		<?php foreach ( $demos as $demo => $demo_details ) : ?>
			...
			<div class="theme <?php echo esc_attr( $demo_details['filter'] ); ?>">
				<div class="theme-wrapper">
					...
					<h3 class="theme-name" id="<?php echo esc_attr( $demo ); ?>" data-live-url="<?php echo esc_url( $demo_url ); ?>"><?php echo wolmart_escaped( $demo_details['alt'] ); ?></h3>
					<a class="demo-button demo-preview fas fa-search" href="<?php echo esc_url( $demo_url ); ?>" target="_blank" title="<?php esc_attr_e( 'Preview', 'wolmart' ); ?>"><?php esc_html_e( 'Preview', 'wolmart' ); ?></a>
					<a class="demo-button demo-import fas fa-download" href="#" title="<?php esc_attr_e( 'Import', 'wolmart' ); ?>"><?php esc_html_e( 'Import', 'wolmart' ); ?></a>
					...
					<?php if ( ! empty( $uninstalled_demo_plugins ) ) : ?>
							<?php foreach ( $uninstalled_demo_plugins as $plugin ) : ?>
								<?php if ( $plugin['required'] || isset( $wolmart_plugins_obj->demo_plugin_dependencies[ $demo ][ $plugin['slug'] ] ) ) : ?>
								<li data-plugin="<?php echo esc_attr( $plugin['slug'] ); ?>">
									...
									<div>
										...
										<a href="#" data-slug="<?php echo esc_attr( $plugin['slug'] ); ?>" data-callback="install_plugin" class="demo-plugin"><?php esc_html_e( 'Install', 'wolmart' ); ?></a>
									</div>
								</li>
							<?php endif; ?>
							<?php endforeach; ?>
						...
						<?php foreach ( $demo_details['editors'] as $editor ) : ?>
							<?php if ( is_plugin_inactive( $all_plugins[ $editor ]['url'] ) ) : ?>
								<li data-plugin="<?php echo esc_attr( $all_plugins[ $editor ]['slug'] ); ?>" class="plugin-editor">
									...
									<a href="#" data-slug="<?php echo esc_attr( $all_plugins[ $editor ]['slug'] ); ?>" data-callback="install_plugin" class="demo-plugin"><?php esc_html_e( 'Install', 'wolmart' ); ?></a>
									</div>
								</li>
							<?php endif; ?>
						<?php endforeach; ?>
