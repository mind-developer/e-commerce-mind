...
class Wolmart_Admin extends Wolmart_Base {
	...
	public function add_theme_update_url() {
		...
		if ( 'update-core.php' == $pagenow ) {
			...
			if ( version_compare( WOLMART_VERSION, $new_version, '<' ) ) {
				...
				wp_add_inline_script( 'wolmart-admin', 'if (jQuery(\'#checkbox_' . $checkbox_id . '\').length) {jQuery(\'#checkbox_' . $checkbox_id . '\').closest(\'tr\').children().last().append(\'<a href="' . esc_url( $url ) . '" target="_blank">' . esc_js( esc_html__( 'View Details', 'wolmart' ) ) . '</a>\');}' );
			}
		}
	}
	...
	public function refresh_transients() {
		...
		setcookie( 'wolmart_dismiss_activate_msg', null, -1, '/' );
	}
