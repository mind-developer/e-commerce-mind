...
(function ($) {
    ...
    var LayoutBuilderView = {
        ...
        init: function () {
            ...
            // Delete button
            this.buttonDelete = '<button class="wolmart-condition-button wolmart-condition-delete far fa-trash-alt"></button>';
            // Duplicate button
            this.buttonDuplicate = '<button class="wolmart-condition-button wolmart-condition-duplicate far fa-clone"></button>';
            // Set conditions button
            this.buttonSet = '<button class="wolmart-condition-button wolmart-condition-set fa fa-cog"></button>';
            ...
            $(document.body)
                ...
                .on('change', '.wolmart-scheme-list', this.changeConditionItem.bind(this))
        ...
        onContextMenu: function (e) {
            ...
            html += prefix + 'copy"><i class="far fa-copy"></i>' + wolmart_layout_vars.text_copy + '</a>';
            if (LayoutBuilderModel.clipboard) {
                html += prefix + 'paste"><i class="fa fa-paste"></i>' + wolmart_layout_vars.text_paste + '</a>';
            }

            if (LayoutBuilderModel.canExtendCondition(category)) {
                html += prefix + 'duplicate"><i class="far fa-clone"></i>' + wolmart_layout_vars.text_duplicate + '</a>';
            }

            html += prefix + 'set"><i class="fa fa-cog"></i>' + wolmart_layout_vars.text_options + '</a>';

            html += prefix + 'delete"><i class="far fa-trash-alt"></i>' + wolmart_layout_vars.text_delete + '</a>';
        ...
        getNewConditionUI: function (category, conditionNo = -1) {
            ...
            if (layoutData) {
                ...
                return '<div class="wolmart-layout-item wolmart-layout-item-' + category + '" data-category="' + category + '" data-condition-no="' + conditionNo + '">' +
                    ...
                    '<span class="wolmart-condition-edit-back fa fa-arrow-left"></span>' +
            ...
            if (optionControls) {
                ...
                for (var optionName in optionControls) {
                    ...
                    if ('buttonset' == control.type) {
                        ...
                        control_html += '<label class="fa fa-redo" for="' + name + '_" title="' + wolmart_layout_vars.text_default + '"></label>';
                        control_html += '<input type="radio" name="' + name + '" id="' + name + '_hide" value="hide"' + (optionValue == 'hide' ? ' checked' : '') + '>';
                        control_html += '<label class="fa fa-eye-slash"for="' + name + '_hide"  title="' + wolmart_layout_vars.text_hide + '"></label>';
                        control_html += '<input type="radio" name="' + name + '" id="' + name + '_my" value="my"' + (optionValue && optionValue != 'hide' ? ' checked' : '') + '>';
                        control_html += '<label class="far fa-hdd" for="' + name + '_my" title="' + wolmart_layout_vars.text_my_templates + '"></label>';
                        control_html += '</div>';
                        // control_html += '<div class="wolmart-radio-button-extend">';
                        // control_html += '<a href="#" class="wolmart-add-new-template fa fa-plus"></a>';
