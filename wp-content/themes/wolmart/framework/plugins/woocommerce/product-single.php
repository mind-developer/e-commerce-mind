		...
		if ( is_a( $prev_post, 'WP_Post' ) || is_a( $next_post, 'WP_Post' ) ) {
			...
			if ( is_a( $prev_post, 'WP_Post' ) ) {
				$html             .= '<li class="product-nav-prev">';
					$html         .= '<a href="' . esc_url( get_the_permalink( $prev_post->ID ) ) . '" rel="prev"><i class="w-icon-angle-left"></i>';
			...
			if ( is_a( $next_post, 'WP_Post' ) ) {
				$html             .= '<li class="product-nav-next">';
					$html         .= '<a href="' . esc_url( get_the_permalink( $next_post->ID ) ) . '" rel="next"><i class="w-icon-angle-right"></i>';
						$html     .= ' <span class="product-nav-popup">';
...
if ( ! function_exists( 'wolmart_single_product_sticky_cart_wrap_start' ) ) {
	function wolmart_single_product_sticky_cart_wrap_start() {
		global $wolmart_layout;
		if ( ! wolmart_doing_ajax() && apply_filters( 'wolmart_single_product_sticky_cart_enabled', ! empty( $wolmart_layout['single_product_sticky'] ) ) ) {
...
if ( ! function_exists( 'wolmart_single_product_sticky_cart_wrap_end' ) ) {
	function wolmart_single_product_sticky_cart_wrap_end() {
		...
		if ( ! wolmart_doing_ajax() && ! $product->is_type( 'external' ) && apply_filters( 'wolmart_single_product_sticky_cart_enabled', ! empty( $wolmart_layout['single_product_sticky'] ) ) ) {
			echo '</div></div>';
		}
	}
}
	...
	function wolmart_single_product_images() {
		...
		if ( 'vertical' == $single_product_layout || 'horizontal' == $single_product_layout ) {
			...
			$html = '<div class="product-gallery-carousel slider-wrapper' .
				...
				. '" data-slider-status="slider-same-height slider-nav-inner slider-nav-fade"' . apply_filters( 'wolmart_single_product_gallery_type_attr', '' ) . '>' . $html . '</div>';
